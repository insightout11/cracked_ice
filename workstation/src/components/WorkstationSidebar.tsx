...
export function WorkstationSidebar() {
  ...
  const { mode, setMode, isPremium } = useWorkstationMode();

  const modes = [
    {
      id: 'ice-level' as const,
      name: 'Ice Level',
      icon: Layers,
      path: '/ice-level',
  ...
  const handleModeClick = (modeId: typeof mode, path: string) => {
    setMode(modeId);
    navigate(path);
  };
          ...
          {modes.map((m) => {
            ...
            return (
              ...
              >
                ...
                {m.premium && !isPremium && (
                  <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-yellow-500 flex items-center justify-center text-[10px]">
                    ⭐
                  </div>
                )}
        ...
        {modes.map((m) => {
          ...
          return (
            ...
            >
              ...
              {m.premium && !isPremium && (
                <div className="absolute top-1 right-[30%] w-3 h-3 rounded-full bg-yellow-500 flex items-center justify-center text-[8px]">
                  ⭐
                </div>
              )}
            </button>
