...
5. Video (optional): `node scripts/weekly/render-video.mjs --start YYYY-MM-DD` renders one storyboard still per scene into `content/social/<season>/video/`; add `--video` for the MP4 (1080x1920, about 32 seconds, about a minute to render). The Remotion project is `video/` (`npm install` there once; `npm run studio` to preview and tweak). Scenes and their lengths are in `video/src/scenes.json`; props come from `scripts/weekly/video-props.mjs`, built from the same weekly JSON and editorial file. Renders aren't committed. Music is added in the app at upload, never baked in, and there's no game footage (NHL rights).
