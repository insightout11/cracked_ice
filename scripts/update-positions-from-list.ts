...
const positionUpdates: Record<string, string[]> = {
  "Jamie Benn": ["C", "LW", "RW"],
  "Mitch Marner": ["LW", "RW"],
  "Thomas Novak": ["C", "LW"],
  "Alex Steeves": ["C", "LW"],
...
const playersPath = path.join(__dirname, '../apps/api/src/data/players.json');
const playersData: PlayersData = JSON.parse(fs.readFileSync(playersPath, 'utf-8'));

// Load current multi-position-players.json
const multiPosPath = path.join(__dirname, '../data/multi-position-players.json');
const multiPosData = fs.existsSync(multiPosPath)
  ? JSON.parse(fs.readFileSync(multiPosPath, 'utf-8'))
...
fs.writeFileSync(multiPosPath, JSON.stringify(multiPosData, null, 2) + '\n', 'utf-8');
