...
function indexMatchups(schedule: SeasonScheduleData, timezone: string): Map<string, Map<string, PublicMatchup>> {
  ...
  // NHL schedule dates are calendar identifiers, not instants. Keep them aligned
  // with the Schedule page; timezone conversion belongs only on puck-drop times.
  void timezone;
...
export function buildPublicBriefing(schedule: SeasonScheduleData, date: string, timezone = 'UTC'): PublicBriefing {
  ...
  const nextMatchups = nextGameDate
    ? [...(indexed.get(nextGameDate)?.values() ?? [])].sort((a, b) => (a.startTime ?? '').localeCompare(b.startTime ?? '') || a.away.localeCompare(b.away))
    : [];
