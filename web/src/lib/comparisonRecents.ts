export interface RecentComparisonPlayer { id: string; name: string }
export interface RecentComparison { playerA: RecentComparisonPlayer; playerB: RecentComparisonPlayer; savedAt: string }
...
export function saveRecentComparison(leagueId: string, comparison: Omit<RecentComparison, 'savedAt'>): void {
  try {
    ...
    // URL state remains canonical if storage is unavailable.
  }
}
