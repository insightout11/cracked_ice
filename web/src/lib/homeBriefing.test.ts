...
describe('home lineup capacity', () => {
  ...
  it.each(['W', 'U', 'FLEX'])('fits a left wing into a %s slot', (slot) => {
  ...
  it('excludes long-term injured players from active capacity', () => {
    const base = createDefaultLeagueWorkspace({ id: 'capacity-ir-lt' });
    const workspace = { ...base, rosterRules: { slots: { LW: 1, 'IR-LT': 1 }, lockingMode: 'daily' as const }, roster: [
      { playerId: 'wing', fullName: 'Wing', team: 'TOR', positions: ['LW'], slot: 'IR-LT', keeper: false, protected: false, undroppable: false },
    ] };
    expect(calculateHomeCapacity(workspace, schedule, '2026-10-10', 'UTC')).toMatchObject({ scheduledSkaters: 0, skaterCapacity: 0, conflict: 0 });
  });
});
