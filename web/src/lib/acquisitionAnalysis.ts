...
export function simulateDailyLineup(
  ...
  const eligibleRoster = roster.filter((player) => {
    const savedSlot = entryById.get(normalizeId(player.id))?.slot ?? player.current_slot;
  ...
  if (workspace.rosterRules.lockingMode === 'weekly') {
    ...
    for (const periodDates of periods.values()) {
      const available = eligibleRoster.map((player) => {
        ...
        return { player, value: (projection?.fppg ?? 0) * games, slots: slotTypes.filter((slot) => canPositionsFillSlot(player.positions, slot)) };
  ...
  dates.forEach((date) => {
    const available = eligibleRoster
      ...
      .map((player) => ({
        ...
        slots: slotTypes.filter((slot) => canPositionsFillSlot(player.positions, slot)),
      }))
...
export function rankAddDropPairs(
  ...
  const droppable = roster.filter((player) => {
    ...
    const slot = entry?.slot ?? player.current_slot;
