...
import { LeagueWorkspaceProvider, useLeagueWorkspace } from './contexts/LeagueWorkspaceContext';
...
function ContextGate({ children }: { children: ReactNode }) {
  ...
  if (!exists) return (
    <main className="mx-auto max-w-xl px-4 py-12">
      <section className="rounded-xl border border-warning/50 bg-surface-1 p-6">
        <p className="scoreboard-text text-warning">LEAGUE CONTEXT UNAVAILABLE</p>
        <h1 className="mt-2 text-2xl font-semibold text-ink">Select a saved league to continue</h1>
        <p className="mt-2 text-sm text-ink-dim">The league referenced by this Home action is not available on this device. Cracked Ice has not substituted a different league.</p>
        <div className="mt-5 grid gap-2">
          {store.leagues.map((league) => (
            <button key={league.id} type="button" onClick={() => { setActiveLeague(league.id); navigate({ pathname: location.pathname, search: replaceLeagueContext(location.search, league.id) }, { replace: true }); }} className="flex min-h-11 items-center justify-between rounded-md border border-line bg-surface-0 px-4 text-left text-sm font-semibold text-ink hover:border-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">
              <span>{league.name}</span><span className="text-xs font-normal text-ink-mute">{league.season.label}</span>
            </button>
          ))}
        </div>
      </section>
