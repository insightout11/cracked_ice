...
import { ArrowDown, ArrowUp, CalendarRange, CalendarX2, Moon, Repeat2 } from 'lucide-react';
...
import { buildFantasySeasonOpportunity, loadSeasonSchedule, type SeasonScheduleData } from '../../lib/schedulePlanning';

type AnalysisMode = 'off-nights' | 'back-to-backs' | 'post-season';
...
export function SeasonAnalysisPanel() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    Promise.all([apiService.getOffNights(start, end), apiService.getBackTobacks(start, end), loadSeasonSchedule()])
      .then(([offNightRows, backToBackRows, seasonSchedule]) => {
        if (cancelled) return;
  ...
  const rows = useMemo<AnalysisRow[]>(() => {
    const teamNames = new Map([...offNights, ...backToBacks].map((team) => [team.teamCode, team.teamName]));
    const raw = mode === 'off-nights'
      ...
      : mode === 'back-to-backs'
        ...
        : schedule
          ? Object.values(buildFantasySeasonOpportunity(schedule, activeLeague)).map((team) => ({ teamCode: team.team, teamName: teamNames.get(team.team) ?? team.team, metric: team.afterFantasySeason, games: team.fantasyRelevantGames }))
          : [];
  ...
  }, [activeLeague, backToBacks, mode, offNights, schedule]);
      ...
      <section className="rounded-xl border border-line-strong bg-surface-glass p-4 shadow-card">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex gap-1 rounded-lg border border-line bg-surface-0 p-1" aria-label="Season analysis metric">
            ...
            <Button size="sm" variant={mode === 'post-season' ? 'primary' : 'ghost'} aria-pressed={mode === 'post-season'} onClick={() => setMode('post-season')}><CalendarX2 size={14} aria-hidden="true" />After playoffs</Button>
          </div>
        </div>
        <div className="mt-4 border-t border-line pt-4">{mode === 'post-season'
          ? <div className="rounded-lg border border-warning/40 bg-warning-muted p-3 text-sm text-ink-dim"><strong className="text-warning">Fantasy cutoff: {activeLeague.schedule.playoffs.end}.</strong> NHL games after your saved championship cannot score in a head-to-head league. Change this date in My League if your format runs through the NHL season.</div>
          : <TimeWindow value={timeWindow.state} onPresetChange={timeWindow.setPreset} onCustomRangeChange={timeWindow.setCustomRange} onModeChange={timeWindow.setMode} onPlayoffPresetChange={timeWindow.setPlayoffPreset} onLeagueWeeksChange={timeWindow.setLeagueWeeks} />}</div>
      </section>
          ...
          <section className="grid gap-3 sm:grid-cols-3" aria-label={`Top ${mode}`}>
            {podium.map((team, index) => (
              <article key={team.teamCode} className={`rounded-xl border p-4 ${index === 0 ? 'border-accent bg-accent-muted shadow-accent' : 'border-line bg-surface-1'}`}>
                ...
                <p className="mt-3 text-xs text-ink-dim">{team.delta >= 0 ? '+' : ''}{team.delta.toFixed(1)} vs league average · {team.games} {mode === 'post-season' ? 'games before cutoff' : 'games'}</p>
              </article>
            ))}
          </section>

          <section className="overflow-hidden rounded-xl border border-line-strong bg-surface-1">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line px-4 py-3"><div><h2 className="font-semibold text-ink">All 32 teams</h2><p className="text-xs text-ink-mute">{mode === 'off-nights' ? 'Off-nights have eight or fewer NHL games league-wide.' : mode === 'back-to-backs' ? 'Back-to-backs are games on consecutive calendar days.' : 'A higher number means more scheduled games arrive after your fantasy championship and cannot contribute.'}</p></div><span className="inline-flex items-center gap-1 text-xs text-ink-mute"><CalendarRange size={13} className="text-accent" aria-hidden="true" />{mode === 'post-season' ? `Fantasy cutoff ${activeLeague.schedule.playoffs.end}` : `${start} to ${end}`}</span></div>
            <div className="max-h-[700px] overflow-auto">
              <table className="w-full min-w-[34rem] text-left text-sm">
                <caption className="sr-only">All NHL teams ranked by {mode}</caption>
                <thead className="sticky top-0 z-10 bg-surface-0 text-xs uppercase tracking-wide text-ink-mute"><tr><th className="px-4 py-3">Team</th>{([['metric', mode === 'off-nights' ? 'Off-nights' : mode === 'back-to-backs' ? 'Back-to-backs' : 'After playoffs'], ['delta', 'Vs average'], ['games', mode === 'post-season' ? 'Before cutoff' : 'Games']] as const).map(([key, label]) => <th key={key} className="px-4 py-3"><button type="button" onClick={() => changeSort(key)} className="inline-flex items-center gap-1 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">{label}{sort.key === key && <SortIcon size={12} aria-hidden="true" />}</button></th>)}</tr></thead>
                <tbody className="divide-y divide-line">{sortedRows.map((team, index) => <tr key={team.teamCode} className={index % 2 ? 'bg-surface-2/40' : 'bg-surface-1'}><td className="px-4 py-2"><div className="flex items-center gap-2"><img src={getTeamLogoUrl(team.teamCode)} alt="" className="size-7 object-contain" onError={(event) => { event.currentTarget.hidden = true; }} /><span><strong className="text-ink">{team.teamCode}</strong><span className="ml-2 hidden text-xs text-ink-mute sm:inline">{team.teamName}</span></span></div></td><td className={`scoreboard-number px-4 py-2 text-base ${mode === 'post-season' && team.metric > 0 ? 'text-warning' : 'text-ink'}`}>{team.metric}</td><td className={`scoreboard-number px-4 py-2 ${team.delta > 0 ? (mode === 'post-season' ? 'text-negative' : 'text-positive') : team.delta < 0 ? (mode === 'post-season' ? 'text-positive' : 'text-negative') : 'text-ink-mute'}`}>{team.delta >= 0 ? '+' : ''}{team.delta.toFixed(1)}</td><td className="scoreboard-number px-4 py-2 text-ink-dim">{team.games}</td></tr>)}</tbody>
              </table>
            </div>
          </section>
        </>
