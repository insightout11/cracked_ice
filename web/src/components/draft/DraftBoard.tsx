      ...
      <div className="flex min-w-0 flex-col gap-4">
        ...
        <Card className="draft-player-pool order-1 overflow-hidden sm:order-3">
          <div className="border-b border-line p-4 sm:p-5">
            ...
            <p className="mt-3 text-[10px] text-ink-mute max-sm:hidden"><strong className="text-ink-dim">Above replacement</strong> is projected FPPG beyond the replacement level at the player's best eligible position. <strong className="text-ink-dim">CI projected FPPG</strong> is the early estimate using your league scoring. <strong className="text-ink-dim">PO starts</strong> means usable starts across all playoff weeks.</p>
            ...
            <details className="mt-2 text-[10px] text-ink-mute sm:hidden"><summary className="cursor-pointer font-semibold text-accent">What do these numbers mean?</summary><p className="mt-2"><strong className="text-ink-dim">CI FPPG</strong> is the early league-scored projection. <strong className="text-ink-dim">PO starts</strong> is usable playoff starts. <strong className="text-ink-dim">Value</strong> is Yahoo ADP minus CI rank; positive means Yahoo usually lets you wait.</p></details>
          </div>
...
function DraftPlayerRow({ candidate, context, selected, targeted, onSelect, onTarget, onMine, onTaken }: { candidate: RankedDraftCandidate; context?: ReturnType<typeof buildDraftCandidateContext> extends Map<string, infer T> ? T : never; selected: boolean; targeted: boolean; onSelect: () => void; onTarget: () => void; onMine: () => void; onTaken: () => void }) {
  return <article className={`grid gap-3 border-l-2 px-3 py-3 transition-colors sm:px-5 lg:grid-cols-[minmax(13rem,1.3fr)_repeat(4,minmax(4.25rem,0.5fr))_minmax(8rem,0.8fr)_auto] lg:items-center ${selected ? 'border-l-accent bg-accent-muted/40' : 'border-l-transparent hover:bg-surface-0/60'}`}>
    ...
    <div className="grid grid-cols-4 gap-1.5 lg:contents">
      ...
      <BoardMetric label="CI proj. FPPG" mobileLabel="CI FPPG" value={candidate.score.metrics.projectedFppg.toFixed(2)} />
