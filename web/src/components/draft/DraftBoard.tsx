...
import { DraftPlayerProfileModal } from './DraftPlayerProfileModal';
...
export function DraftBoard() {
  ...
  const recommendationRankings = useMemo(() => rankings.filter((candidate) =>
    ...
    && (position === 'ALL' || candidate.player.pos.includes(position))), [baseCandidateIds, position, rankings]);
  ...
  const visibleRankings = useMemo(() => rankings.filter((candidate) =>
    (position === 'ALL' || candidate.player.pos.includes(position))
  ...
  const visibleTiers = useMemo(() => tiers.map((tier) => ({
    ...
    candidates: tier.candidates.filter((candidate) =>
      ...
      && (position !== 'ALL' || contextById.get(normalizeId(candidate.player.id))?.position === tier.position)),
  ...
  </div>{profileCandidate && <DraftPlayerProfileModal candidate={profileCandidate} context={contextById.get(normalizeId(profileCandidate.player.id))} market={marketById.get(normalizeId(profileCandidate.player.id))} statsSeason={meta?.statsSeason} onClose={() => setProfileId(null)} />}</>;
  ...
  return <div className="mx-auto max-w-[1500px] space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <div className="border-b border-line p-4 sm:p-5">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
                ...
                <div className="inline-flex flex-wrap rounded-lg border border-line bg-surface-0 p-1" aria-label="Draft board position filter">{POSITIONS.map((item) => <button key={item} type="button" onClick={() => setPosition(item)} className={`rounded-md px-2.5 py-2 text-xs font-semibold ${position === item ? 'bg-accent text-accent-ink' : 'text-ink-dim hover:text-ink'}`}>{item}</button>)}</div>
      ...
      <aside className="space-y-4 xl:sticky xl:top-4 xl:self-start">
        ...
        <Card className="hidden overflow-hidden xl:block"><div className="border-b border-line p-4"><p className="scoreboard-text text-accent">PLAYER INFO</p><h2 className="text-base font-semibold text-ink">Decision context</h2></div>{selected ? <SelectedPlayer candidate={selected} context={contextById.get(normalizeId(selected.player.id))} market={marketById.get(normalizeId(selected.player.id))} targeted={targetById.has(normalizeId(selected.player.id))} onAdjust={(delta) => adjustPlayerRank(selected.player.id, delta)} onFullProfile={() => setProfileId(selected.player.id)} onTarget={() => toggleTarget(selected)} onMine={() => markPlayer(selected, 'mine')} onTaken={() => markPlayer(selected, 'taken')} /> : <p className="p-4 text-sm text-ink-dim">Select a player to inspect their league and schedule fit.</p>}</Card>
    ...
    {profileCandidate && <DraftPlayerProfileModal candidate={profileCandidate} context={contextById.get(normalizeId(profileCandidate.player.id))} market={marketById.get(normalizeId(profileCandidate.player.id))} statsSeason={meta?.statsSeason} onClose={() => setProfileId(null)} />}
...
function ProjectionDisclosure({ meta }: { meta: DraftPlayerDirectoryMeta | null }) {
  ...
  return <details className="mt-2 rounded-lg border border-line bg-surface-0 px-3 py-2 text-[10px] text-ink-mute open:border-line-strong">
    ...
    <div className="mt-2 grid gap-2 border-t border-line pt-2 sm:grid-cols-2">
      ...
      <p><strong className="text-ink">Current limits:</strong> {DRAFT_PROJECTION_MODEL.limitations}</p>
    </div>
...
function CompactDraftRow({ candidate, label, context, targeted, onSelect, onTarget, onMine, onTaken }: { candidate: RankedDraftCandidate; label?: string; context?: ReturnType<typeof buildDraftCandidateContext> extends Map<string, infer T> ? T : never; targeted: boolean; onSelect: () => void; onTarget: () => void; onMine: () => void; onTaken: () => void }) {
  return <article className="p-3 sm:p-4">
    ...
    <div className="mt-3 grid grid-cols-4 gap-1.5"><MiniMetric value={formatYahooAdp(candidate.player)} label="Yahoo ADP" /><MiniMetric value={candidate.score.metrics.projectedFppg.toFixed(2)} label="CI early proj." /><MiniMetric value={`${candidate.score.metrics.playoffUsableStarts}/${candidate.score.metrics.championshipWeek.usableStarts}`} label="PO / final" /><MiniMetric value={context?.dropToNextAtPosition ?? 0} label="next drop" /></div>
...
function RankedDraftList({ candidates, marketById, contextById, selectedId, targetById, onSelect, onTarget, onMine, onTaken }: { candidates: RankedDraftCandidate[]; marketById: Map<string, DraftMarketContext>; contextById: Map<string, DraftCandidateContext>; selectedId: string | null; targetById: Map<string, LeagueWorkspace['draftSession']['targets'][number]>; onSelect: (id: string) => void; onTarget: (candidate: RankedDraftCandidate) => void; onMine: (candidate: RankedDraftCandidate) => void; onTaken: (candidate: RankedDraftCandidate) => void }) {
  return <div>
    <div className="sticky top-[8.5rem] z-10 hidden grid-cols-[3rem_minmax(13rem,1.4fr)_repeat(5,minmax(4.5rem,0.55fr))_minmax(9rem,0.8fr)_auto] items-center gap-3 border-b border-line bg-surface-2/95 px-5 py-2 text-[9px] font-bold uppercase tracking-wide text-ink-mute [backdrop-filter:var(--frost)] lg:grid"><span>Rank</span><span>Player</span><span>Value vs ADP</span><span>Yahoo ADP</span><span>Draft score</span><span>CI projection</span><span>PO / final</span><span>Decision</span><span className="sr-only">Actions</span></div>
...
function RankedDraftRow({ candidate, context, market, selected, targeted, onSelect, onTarget, onMine, onTaken }: { candidate: RankedDraftCandidate; context?: DraftCandidateContext; market?: DraftMarketContext; selected: boolean; targeted: boolean; onSelect: () => void; onTarget: () => void; onMine: () => void; onTaken: () => void }) {
  ...
  return <article className={`border-l-2 px-4 py-3 transition-colors [contain-intrinsic-size:auto_84px] [content-visibility:auto] sm:px-5 ${selected ? 'border-l-accent bg-accent-muted/40' : 'border-l-transparent hover:bg-surface-0/60'}`}>
    <div className="grid gap-3 lg:grid-cols-[3rem_minmax(13rem,1.4fr)_repeat(5,minmax(4.5rem,0.55fr))_minmax(9rem,0.8fr)_auto] lg:items-center">
      ...
      <RankedMetric label="CI early projection FPPG" value={candidate.score.metrics.projectedFppg.toFixed(2)} />
      <RankedMetric label="Playoffs / final" value={`${candidate.score.metrics.playoffUsableStarts}/${candidate.score.metrics.championshipWeek.usableStarts}`} tone="positive" />
...
function SelectedPlayer({ candidate, context, market, targeted, onAdjust, onFullProfile, onTarget, onMine, onTaken }: { candidate: RankedDraftCandidate; context?: DraftCandidateContext; market?: DraftMarketContext; targeted: boolean; onAdjust: (delta: number) => void; onFullProfile: () => void; onTarget: () => void; onMine: () => void; onTaken: () => void }) {
