...
import { assignDraftSlot, buildDraftCandidateContext, buildDraftTiers, currentDraftRound, readDraftRoomLayout, withDraftRoomLayout } from '../../lib/draftRoom';
...
export function DraftBoard() {
  ...
  const urgent = rankings.slice(0, 24).filter((candidate) => contextById.get(normalizeId(candidate.player.id))?.advice === 'take-now').sort((a, b) => (contextById.get(normalizeId(b.player.id))?.dropToNextAtPosition ?? 0) - (contextById.get(normalizeId(a.player.id))?.dropToNextAtPosition ?? 0))[0];
  const playoff = [...rankings.slice(0, 30)].sort((a, b) => b.score.metrics.playoffUsableStarts - a.score.metrics.playoffUsableStarts || b.score.total - a.score.total)[0];
  const recommendations = [rankings[0], urgent, playoff].filter((candidate, index, list): candidate is RankedDraftCandidate => Boolean(candidate) && list.findIndex((item) => item?.player.id === candidate?.player.id) === index);
  ...
  return <div className="mx-auto max-w-[1500px] space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="space-y-4 xl:sticky xl:top-4 xl:self-start">
        ...
        <Card className="hidden overflow-hidden xl:block"><div className="border-b border-line p-4"><p className="scoreboard-text text-accent">PLAYER INFO</p><h2 className="text-base font-semibold text-ink">Decision context</h2></div>{selected ? <SelectedPlayer candidate={selected} context={contextById.get(normalizeId(selected.player.id))} targeted={targetById.has(normalizeId(selected.player.id))} onTarget={() => toggleTarget(selected)} onMine={() => markPlayer(selected, 'mine')} onTaken={() => markPlayer(selected, 'taken')} /> : <p className="p-4 text-sm text-ink-dim">Select a player to inspect their league and schedule fit.</p>}</Card>
...
function CompactDraftRow({ candidate, label, context, targeted, onSelect, onTarget, onMine, onTaken }: { candidate: RankedDraftCandidate; label?: string; context?: ReturnType<typeof buildDraftCandidateContext> extends Map<string, infer T> ? T : never; targeted: boolean; onSelect: () => void; onTarget: () => void; onMine: () => void; onTaken: () => void }) {
  return <article className="p-3 sm:p-4">
    ...
    <div className="mt-3 grid grid-cols-3 gap-1.5"><MiniMetric value={candidate.score.metrics.fppg.toFixed(2)} label="FPPG" /><MiniMetric value={`${candidate.score.metrics.playoffUsableStarts}/${candidate.score.metrics.championshipWeek.usableStarts}`} label="PO / final" /><MiniMetric value={context?.dropToNextAtPosition ?? 0} label="next drop" /></div>
...
function RecommendationCard({ candidate, label, context, onSelect, onMine }: { candidate: RankedDraftCandidate; label: string; context?: ReturnType<typeof buildDraftCandidateContext> extends Map<string, infer T> ? T : never; onSelect: () => void; onMine: () => void }) {
  return <article className="rounded-xl border border-line bg-surface-0 p-3"><div className="flex items-center justify-between gap-2"><span className="scoreboard-text text-accent">{label}</span><span className="rounded-full bg-accent-muted px-2 py-1 text-[10px] font-bold text-accent">Tier {context?.tier ?? '—'}</span></div><button type="button" onClick={onSelect} className="mt-3 w-full text-left"><PlayerIdentity player={candidate.player} /></button><div className="mt-3 grid grid-cols-3 gap-1.5"><MiniMetric value={candidate.score.total.toFixed(1)} label="score" /><MiniMetric value={candidate.score.metrics.fppg.toFixed(2)} label="FPPG" /><MiniMetric value={`${candidate.score.metrics.playoffUsableStarts}/${candidate.score.metrics.championshipWeek.usableStarts}`} label="PO / final" /></div><p className={`mt-3 text-xs ${context?.advice === 'take-now' ? 'text-warning' : 'text-ink-dim'}`}>{context?.advice === 'take-now' ? `Take now · ${context.dropToNextAtPosition} point drop at position` : `${context?.similarAtPosition ?? 0} comparable option${context?.similarAtPosition === 1 ? '' : 's'} remain`}</p><button type="button" onClick={onMine} className="mt-3 inline-flex min-h-9 w-full items-center justify-center gap-1.5 rounded-lg bg-accent text-xs font-bold text-accent-ink"><Check size={14} />Draft to my team</button></article>;
}
