...
export function DraftBoard() {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-4">
    ...
    <Card className="p-4"><DraftStrategyControl value={activeLeague.draftStrategy} onChange={(draftStrategy) => updateLeague({ ...activeLeague, draftStrategy, updatedAt: new Date().toISOString() })} /><p className="mt-3 text-xs text-ink-mute">{meta?.statsSeason ?? 'Prior-season'} NHL stats · {meta?.eligibilitySource === 'yahoo' ? `Yahoo ADP and eligibility updated ${meta.eligibilityUpdatedAt ?? 'date unavailable'} · ` : ''}default pool requires 20 skater GP / 25 goalie GP · rankings update after every recorded pick</p></Card>
