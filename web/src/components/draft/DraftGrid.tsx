...
export function DraftGrid({ workspace, onDraftPositionChange, onRemovePick, onTeamNameChange, availabilityPick, onAvailabilityPickChange }: DraftGridProps) {
  ...
  const orderLabel = workspace.draftSession.orderType === 'balanced'
    ? 'balanced'
    : workspace.draftSession.orderType;

  return <div>
    <div className="flex flex-col gap-3 border-b border-line p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
      <p className="max-w-2xl text-xs text-ink-dim">{workspace.draftSession.mode === 'planner' ? 'Mine fills your next open slot; Taken and simulations fill opponent slots.' : `Every Taken or Mine action lands in the next open ${orderLabel}-draft cell.`} Use the grid to see positional runs, roster shapes, and who picks around you.</p>
