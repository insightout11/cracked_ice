...
import type { DraftOrderType, LeagueWorkspace } from '../../lib/leagueWorkspace';
import { DRAFT_MARKETS, type DraftMarketSource } from '../../lib/draftMarket';
...
interface DraftPlannerPanelProps {
  ...
  pickClockHours: number | null;
  marketSource: DraftMarketSource;
  marketLabel: string;
  canUseKkupflMarket: boolean;
  ...
  onOrderTypeChange: (orderType: DraftOrderType) => void;
  onMarketSourceChange: (source: DraftMarketSource) => void;
...
export function DraftPlannerPanel({ mode, projectionLabel, hasDraftPosition, pickCount, myPickCount, simulatedPickCount, numberOfTeams, draftPosition, orderType, draftStartDate, pickClockHours, marketSource, marketLabel, canUseKkupflMarket, summary, availability, availabilityCurves, availabilityTargets, availabilityPick, availabilityQuery, targets, onModeChange, onTeamCountChange, onDraftPositionChange, onOrderTypeChange, onMarketSourceChange, onSimulateToNext, onSimulateRest, onReroll, onReset, onApplyRoster, onAvailabilityPickChange, onAvailabilityQueryChange, onAddTargetAtPick }: DraftPlannerPanelProps) {
  ...
  return <section className="overflow-hidden rounded-xl border border-line bg-surface-1" aria-label="Draft mode and planning controls">
    ...
    {planner && <>
      ...
      {myPickCount > 0 && <div className="flex flex-col gap-3 border-b border-positive/30 bg-positive-muted/35 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold text-positive">Your {myPickCount} planned selection{myPickCount === 1 ? ' is' : 's are'} ready for My Team</p><p className="mt-0.5 text-[10px] text-ink-dim">Only picks marked Mine are added. Simulated and recorded opponent picks stay out.</p></div><div className="flex shrink-0 gap-2">{confirmingApply ? <><button type="button" onClick={() => { onApplyRoster(); setConfirmingApply(false); }} className="inline-flex min-h-9 items-center gap-1.5 rounded-md bg-positive px-3 text-xs font-bold text-white"><Check size={13} />Confirm My Team update</button><button type="button" onClick={() => setConfirmingApply(false)} className="min-h-9 rounded-md border border-line px-3 text-xs font-semibold text-ink-dim">Cancel</button></> : <button type="button" onClick={() => setConfirmingApply(true)} className="inline-flex min-h-9 items-center gap-1.5 rounded-md border border-positive/60 bg-surface-0 px-3 text-xs font-bold text-positive"><Check size={13} />Add my {myPickCount} selection{myPickCount === 1 ? '' : 's'} to My Team</button>}</div></div>}

      {availabilityTargets.length > 0 && <div id="draft-availability" className="border-b border-line p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="flex items-center gap-2"><BarChart3 size={15} className="text-accent" /><div><h3 className="text-sm font-semibold text-ink">Availability at {selectedTarget ? `pick #${selectedTarget.overallPick}` : 'a future pick'}</h3><p className="text-[10px] text-ink-mute">500 {marketLabel}-based rooms using the players still available after keepers and recorded picks. Estimates are not guarantees.</p></div></div><label className="relative block min-w-56"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-mute" size={14} /><span className="sr-only">Search availability players</span><input value={availabilityQuery} onChange={(event) => onAvailabilityQueryChange(event.target.value)} placeholder="Find a player or team" className="min-h-9 w-full rounded-md border border-line bg-surface-0 pl-9 pr-3 text-xs text-ink outline-none focus:border-accent" /></label></div>
        ...
        {normalizedAvailabilityQuery && availabilityCurves.length > 0
          ? <div className="mt-3 space-y-3">{availabilityMatches.slice(0, 5).map((item) => item.yahooAdp === null
            ? <ManualTargetCard key={item.playerId} item={item} marketLabel={marketLabel} targets={availabilityTargets} assignedTarget={targets.find((target) => target.playerId === item.playerId)} onAddTarget={onAddTargetAtPick} />
            : <AvailabilityCurveCard key={item.playerId} item={item} marketLabel={marketLabel} curve={availabilityCurves.find((curve) => curve.playerId === item.playerId)} selectedPick={selectedTarget?.overallPick ?? null} assignedTarget={targets.find((target) => target.playerId === item.playerId)} compareSelected={compareIds.includes(item.playerId)} compareDisabled={compareIds.length >= 2 && !compareIds.includes(item.playerId)} onCompare={() => toggleCompare(item.playerId)} onPickChange={onAvailabilityPickChange} onAddTarget={onAddTargetAtPick} />)}</div>
          : availabilityMatches.length > 0 ? <div className="mt-3 space-y-4"><AvailabilityGroup title="Decision zone" description="The useful choices for this pick: available often enough to plan for, but risky enough that waiting matters." items={decisionZone} marketLabel={marketLabel} tone="decision" selectedTarget={selectedTarget} targets={targets} compareIds={compareIds} onCompare={toggleCompare} onAddTargetAtPick={onAddTargetAtPick} /><AvailabilityGroup title="Likely available later" description="Probably still on the board here. You may be able to wait instead of reaching now." items={likelyLater} marketLabel={marketLabel} tone="likely" selectedTarget={selectedTarget} targets={targets} compareIds={compareIds} onCompare={toggleCompare} onAddTargetAtPick={onAddTargetAtPick} /><AvailabilityGroup title="Needs to fall" description="Possible only if the room lets them slide. Treat these as bonuses, not the plan." items={needsToFall} marketLabel={marketLabel} tone="fall" selectedTarget={selectedTarget} targets={targets} compareIds={compareIds} onCompare={toggleCompare} onAddTargetAtPick={onAddTargetAtPick} /></div> : <p className="mt-3 rounded-md border border-dashed border-line px-3 py-4 text-center text-xs text-ink-dim">No matching draft-relevant players. Try a name or team.</p>}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-4"><p className="text-xs text-ink-mute">{pickCount} picks in this scenario · {myPickCount} mine · {simulatedPickCount} simulated</p><div className="flex flex-wrap gap-2">{confirmingReset ? <><button type="button" onClick={() => { onReset(); setConfirmingReset(false); }} className="min-h-9 rounded-md bg-negative px-3 text-xs font-bold text-white">Confirm reset</button><button type="button" onClick={() => setConfirmingReset(false)} className="min-h-9 rounded-md border border-line px-3 text-xs font-semibold text-ink-dim">Cancel</button></> : <button type="button" disabled={!pickCount} onClick={() => setConfirmingReset(true)} className="inline-flex min-h-9 items-center gap-1.5 rounded-md border border-negative/50 px-3 text-xs font-semibold text-negative disabled:opacity-40"><Trash2 size={13} />Reset scenario</button>}</div></div>
    </>}
  </section>;
}
...
function AvailabilityGroup({ title, description, items, marketLabel, tone, selectedTarget, targets, compareIds, onCompare, onAddTargetAtPick }: {
  title: string;
  description: string;
...
export function draftComparisonPath(playerIds: string[], planner?: { plannerPick: number | null; plannerSearch: string; from?: 'draft-planner' | 'draft-board'; draftView?: 'tiers' | 'ranked'; draftSearch?: string; draftPosition?: string }): string | null {
  if (playerIds.length !== 2) return null;
  const params = new URLSearchParams({ mode: 'draft', a: playerIds[0], b: playerIds[1], from: planner?.from ?? 'draft-planner' });
  ...
  if (planner?.draftView) params.set('draftView', planner.draftView);
  if (planner?.draftSearch?.trim()) params.set('draftSearch', planner.draftSearch.trim());
  if (planner?.draftPosition && planner.draftPosition !== 'ALL') params.set('draftPosition', planner.draftPosition);
...
function ManualTargetCard({ item, marketLabel, targets, assignedTarget, onAddTarget }: {
  ...
  return <article className="rounded-lg border border-line bg-surface-0 p-3">
    <div><strong className="text-sm text-ink">{item.name}</strong><p className="text-[10px] text-ink-mute">{item.team} · {item.positions.join('/')} · {marketLabel} unavailable</p></div>
    <p className="mt-2 text-[10px] text-ink-dim">There is not enough {marketLabel} data for a defensible availability percentage. Choose a round manually.</p>
...
function AvailabilityCurveCard({ item, marketLabel, curve, selectedPick, assignedTarget, compareSelected, compareDisabled, onCompare, onPickChange, onAddTarget }: {
  item: PlayerAvailabilityEstimate;
  marketLabel: string;
  ...
  return <article className="rounded-lg border border-line bg-surface-0 p-3">
    <div className="flex flex-wrap items-start justify-between gap-2"><div><strong className="text-sm text-ink">{item.name}</strong><p className="text-[10px] text-ink-mute">{item.team} · {item.positions.join('/')} · {marketLabel} {item.yahooAdp?.toFixed(1) ?? '—'}</p></div><div className="flex items-center gap-2">{assignedTarget?.targetOverallPick && <span className="rounded-full border border-warning/50 bg-warning-muted px-2 py-1 text-[9px] font-semibold text-warning">Targeted R{assignedTarget.targetRound} · #{assignedTarget.targetOverallPick}</span>}<button type="button" aria-pressed={compareSelected} disabled={compareDisabled} onClick={onCompare} className={`inline-flex min-h-8 items-center gap-1 rounded-md border px-2 text-[10px] font-semibold disabled:opacity-35 ${compareSelected ? 'border-accent bg-accent-muted text-accent' : 'border-line text-ink-dim'}`}><ArrowLeftRight size={11} />{compareSelected ? 'Selected' : 'Compare'}</button></div></div>
