...
export function MobileMenu({ open, onClose }: MobileMenuProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-surface-0/80 px-4 pt-4 backdrop-blur-md lg:hidden" role="dialog" aria-modal="true" aria-label="Site navigation" onClick={onClose}>
      <div className="mx-auto max-w-md rounded-lg border border-line bg-surface-raised p-4 shadow-raised" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="flex flex-col gap-2" role="navigation" aria-label="Mobile navigation">
          ...
          {TOOL_NAV_ITEMS.map((item) => <NavLink key={item.to} to={item.to} onClick={onClose} className="rounded-md border border-line bg-surface-glass px-4 py-3 text-base font-semibold text-ink hover:border-accent hover:text-accent">{item.label}</NavLink>)}
