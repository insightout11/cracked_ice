...
function MatchupTile({
  ...
  return (
    <article className={`relative flex overflow-hidden rounded-xl border border-line bg-surface-1 ${compact ? 'h-[76px] px-3 py-2' : 'h-[104px] px-4 py-3'}`}>
      <span className="absolute inset-y-0 left-0 w-1" style={{ backgroundColor: getTeamColor(player.team) }} />
      <div className={`relative shrink-0 ${compact ? 'size-12' : 'size-16'}`}>
      ...
      <div className="ml-3 min-w-0 flex-1">
        <p className={`${compact ? 'text-sm' : 'text-lg'} truncate font-black text-ink`}>{player.full_name}</p>
        <div className="mt-1 flex items-center gap-2 font-mono text-xs font-bold text-accent">
          <span>{game.isHome ? 'vs' : '@'}</span>
          <img src={opponentLogoUrl} alt="" crossOrigin="anonymous" className="size-5 object-contain" />
    ...
    <div className="relative flex h-[1350px] w-[1080px] flex-col overflow-hidden bg-surface-0 font-sans text-ink">
      ...
      <section className="relative px-12 pb-5 pt-7">
        ...
        <div className="mt-2 flex items-end justify-between gap-8">
          ...
          <div className="rounded-full border border-accent bg-accent-muted px-5 py-2 text-sm font-bold text-accent">GAME-DAY DECISION</div>
        </div>
            ...
            {groups.map((group) => (
              <section key={group.label}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {group.players.map((item) => <MatchupTile key={item.player.id} item={item} projection={getPlayerProjection(projections, item.player.id)} />)}
                </div>
              </section>
            ))}
      ...
      <footer className="relative mx-12 mt-4 flex items-center justify-between border-t border-line py-6">
        <div>
          <p className="text-base font-black uppercase tracking-wide text-ink">YOUR LEAGUE. TONIGHT&apos;S MATCHUPS. ONE DECISION.</p>
          <p className="mt-1 text-sm text-ink-dim">Share your lineup and settle the start/sit debate.</p>
        </div>
