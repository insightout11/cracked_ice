...
export function ToolsMenu() {
  ...
  return (
    <div className="relative">
      ...
      {open && <div ref={panelRef} role="menu" className="absolute right-0 top-full z-[2100] mt-2 w-52 rounded-lg border border-line-strong bg-surface-raised p-1 shadow-raised">{TOOL_NAV_ITEMS.map((item) => <Link role="menuitem" key={item.to} to={item.to} className="block min-h-11 rounded-md px-3 py-3 text-sm font-semibold text-ink hover:bg-accent-muted hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">{item.label}</Link>)}</div>}
    </div>
  );
}
