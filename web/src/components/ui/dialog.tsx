...
export const ModalContent = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content>>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Portal>
    <DialogPrimitive.Overlay className="fixed inset-0 z-[3100] bg-surface-0/80 backdrop-blur-sm" />
    <DialogPrimitive.Content ref={ref} className={cn('fixed left-1/2 top-1/2 z-[3100] max-h-[85vh] w-[min(92vw,36rem)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-lg border border-line bg-surface-raised p-6 text-ink shadow-raised [backdrop-filter:var(--frost)]', className)} {...props}>
      {children}
...
export const DrawerContent = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content>>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Portal>
    <DialogPrimitive.Overlay className="fixed inset-0 z-[3100] bg-surface-0/80 backdrop-blur-sm" />
    <DialogPrimitive.Content ref={ref} className={cn('fixed inset-y-0 right-0 z-[3100] w-[min(92vw,28rem)] overflow-y-auto border-l border-line bg-surface-raised p-6 text-ink shadow-raised [backdrop-filter:var(--frost)]', className)} {...props}>{children}</DialogPrimitive.Content>
  </DialogPrimitive.Portal>
));
