...
const AVAILABILITY_OPTIONS = [
  { value: 'fa' as const, label: 'Free Agent', color: 'text-green-400 bg-green-500/20' },
  { value: 'owned' as const, label: 'Owned', color: 'text-red-400 bg-red-500/20' },
  { value: 'waivers' as const, label: 'Waivers', color: 'text-yellow-400 bg-yellow-500/20' },
];
    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
          ...
          {/* Team Filter */}
          <div>
            <h3 className="text-sm font-bold text-white mb-3">Teams</h3>
            <button
            ...
            {localFilters.teams.length > 0 && !showTeamPicker && (
              <div className="flex flex-wrap gap-2 mt-2">
                {localFilters.teams.map((team) => (
                  <span
                    key={team}
                    className="px-2 py-1 bg-cyan-600/20 text-cyan-400 text-xs rounded-full font-medium flex items-center gap-1"
                  >
                    {team}
                    <button
                      onClick={() => toggleTeam(team)}
        ...
        <div className="sticky bottom-0 bg-slate-900 border-t border-slate-700 px-4 py-4 safe-area-bottom">
          <button
            onClick={handleApply}
            className="w-full py-4 bg-cyan-600 rounded-xl text-white font-semibold text-base hover:bg-cyan-500 active:bg-cyan-700 transition-colors"
          >
            Apply Filters
          </button>
        </div>
