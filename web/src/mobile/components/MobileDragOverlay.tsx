...
function getIceGlowStyle(score: number): React.CSSProperties {
  ...
  const cyan = `rgba(0, 247, 255, ${0.6 + t * 0.4})`;
...
export function MobileDragOverlay({ player, projection }: MobileDragOverlayProps) {
  ...
  return (
    <DragOverlay dropAnimation={null}>
      <div
        className="bg-slate-800 rounded-lg border-2 border-cyan-400 overflow-hidden shadow-2xl shadow-cyan-500/30"
        style={{
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              <img
                ...
                className="w-10 h-10 rounded-full bg-slate-700 object-cover border-2 border-cyan-400"
                onError={(e) => {
              ...
              <img
                ...
                alt={player.team}
                className="w-6 h-6 rounded-full bg-slate-900 border border-slate-600 p-0.5"
                onError={(e) => {
